    ...
    >
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={viewportConfig}
          ...
          <p style={{ color: "var(--text-muted)", fontSize: 16, marginTop: 16, maxWidth: 560 }}>
            Open to full-time roles, freelance projects, and interesting AI collaborations. Let&rsquo;s build something great.
          </p>
        </motion.div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 64 }} className="contact-grid">
          ...
          >
            {[
              ...
              { icon: <Phone size={18} />,   label: "Phone",    value: personalInfo.phone,    href: `tel:${personalInfo.phone.replace(/\s/g,"")}` },
              { icon: <MapPin size={18} />,  label: "Location", value: personalInfo.location, href: "" },
            ].map((c) => (
              <motion.div key={c.label} variants={staggerItem} className="glass-card"
                style={{ padding: "20px 24px", display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ color: "var(--cyan)", flexShrink: 0 }}>{c.icon}</div>
                <div>
                  <p style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>{c.label}</p>
                  {c.href ? (
                    <a href={c.href} style={{ color: "var(--text)", fontSize: 14, textDecoration: "none", transition: "color 0.2s" }}
                      onMouseEnter={(e) => (e.currentTarget.style.color = "var(--cyan)")}
                      onMouseLeave={(e) => (e.currentTarget.style.color = "var(--text)")}>
                      {c.value}
                    </a>
                  ) : (
                    <p style={{ color: "var(--text)", fontSize: 14 }}>{c.value}</p>
                  )}
                </div>
              </motion.div>
            ))}
            ...
            {/* Social */}
            <motion.div variants={staggerItem} style={{ display: "flex", gap: 12, paddingTop: 8 }}>
              {[
                ...
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ padding: "10px 20px", display: "flex", alignItems: "center", gap: 8, color: "var(--text-muted)", textDecoration: "none", fontSize: 13, fontWeight: 500, transition: "color 0.2s, border-color 0.2s" }}
                  onMouseEnter={(e) => { e.currentTarget.style.color = "var(--cyan)"; }}
      ...
      {/* Footer */}
      <div style={{ textAlign: "center", marginTop: 80, paddingTop: 32, borderTop: "1px solid rgba(0,212,255,0.08)" }}>
        <p style={{ color: "var(--text-muted)", fontSize: 13 }}>
          © 2026 Sivaprasath V. All rights reserved.
        </p>
      </div>
